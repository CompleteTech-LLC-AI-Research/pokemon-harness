...
collected 730 tests. Both runtimes passed unit 586/586 and timing 40/40 in each
...
The bare host `python3` could not create a new standard-library virtual
environment because `ensurepip` was unavailable; the passing gates used
existing managed environments, so clean-install reproducibility remains open.
...
and authenticated/encrypted cross-host TCP also remain open.
...
- current isolated source/Cython unit-timing check: `PASS` for the asset-free
  scope, collection 730, unit 586/586, and timing 40/40 in each of five
...
|---|---|---|
| ROM-free unit and timing regressions | `PASS` (asset-free scoped; release remains `PARTIAL`) | Fresh isolated source and Cython gates each collected 730 tests, passed unit 586/586, and passed timing 40/40 across five repetitions. Source reported `python-source`; Cython reported `cython/native-extension`. No ROM-backed gameplay ran. |
...
| Cython/native serial runtime | `PARTIAL` (current scoped tiers) | The integrated build verifies native modules, the focused serial/link suite passes 107/107, and the native unit/timing gate passes 586/586 and 40/40 × 5 in the isolated asset-free scope. The native strict trade gate passes 18/19; representative native battle rows pass, while full native battle coverage and load stability remain open. |
...
isolated source and Cython unit/timing gates each pass 586/586 and 40/40 x5;

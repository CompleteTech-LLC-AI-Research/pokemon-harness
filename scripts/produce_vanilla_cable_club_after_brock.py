"""Diagnostic-only producer for derived stock Red/Blue Cable Club states.
...
patches the minimum link prerequisites used by the matrix harness. It
intentionally mutates emulator RAM/CPU state and writes a derived save state;
the output is not a human-valid gameplay capture, clean fixture provenance,
or production acceptance evidence. ROM and symbol files must match the
configured ``VERSIONS.md`` pins.
"""
...
def _validated_pins(
    rom: Path,
    sym: Path,
    supplied_sha1: str | None = None,
):
    """Resolve and validate ROM/symbol/runtime pins from ``VERSIONS.md``."""
...
def _close_session(
    ...
    *,
    ...
    """Close the diagnostic session and surface a teardown failure."""
    if session is None:
        return
    try:
        session.close()
    except Exception as exc:  # noqa: BLE001 - cleanup must be best effort
        exc.add_note("diagnostic cleanup failed for vanilla producer session")
        print(
            f"[diagnostic cleanup] failed to close vanilla producer session: "
...
def _poison_blackout_to_cerulean(session: Session, drv: rtb.Driver) -> bool:
    ...
    for i in range(240):
        ...
        if i % 40 == 39:
            ...
            print(
                f"[diagnostic-only] poison step {i + 1}: map=0x{gs.overworld.map_id:02x} "
...
def _special_warp_to_cerulean(session: Session, drv: rtb.Driver) -> bool:
    ...
    print(
        f"[diagnostic-only] special warp: bank1 PrepareForSpecialWarp=0x{prepare:04x} "
    ...
    for phase in range(80):
        ...
        if gs.overworld.map_id == CERULEAN_CITY:
            ...
            print(
                f"[diagnostic-only] arrived Cerulean City by special warp after "
                f"{(phase + 1) * 15} frames",
                flush=True,
            )
            return True
        if phase % 8 == 7:
            print(
                f"[diagnostic-only] special phase {phase + 1}: map=0x{gs.overworld.map_id:02x} "
...
def produce(source: Path, rom: Path, sym: Path, out: Path,
    ...
    try:
        ...
        if not _special_warp_to_cerulean(session, drv) and not _poison_blackout_to_cerulean(session, drv):
            ...
            print(
                f"[diagnostic-only] PC redirected: 0x{pc_before:04x} -> 0x{handle_blackout:04x}",
                flush=True,
            )
            ...
            for phase in range(20):
                ...
                if phase % 4 == 3:
                    print(
                        f"[diagnostic-only] phase {phase + 1}: map=0x{gs.overworld.map_id:02x} "
        ...
        if gs.overworld.map_id != CERULEAN_POKECENTER:
            print(
                f"[diagnostic-only] door warp did not fire; direct EnterMap to Cerulean PC from "
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "optional compatibility assertion; it must equal the configured "
            "VERSIONS.md ROM pin and cannot disable validation"
        ),
    )
    args = ap.parse_args()

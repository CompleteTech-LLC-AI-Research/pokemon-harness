...
class VersionsConfig:
    ...
    def sha1_for_symbol_path(self, path: str | Path) -> str | None:
        """Return the documented SHA-1 for a symbol-file path."""
        candidate = _normalise_path_key(path)
        for documented_path, sha1 in self.symbol_sha1_by_path:
            if candidate == documented_path or candidate.endswith(
                "/" + documented_path
            ):
                return sha1
...
def load_versions(path: str | Path | None = None) -> VersionsConfig:
    ...
    for line in text.splitlines():
        ...
        if path_row is not None and pending_sha is not None:
            ...
            pending_sha = None
            continue
        symbols_row = re.match(r"^\|\s*Symbols\s*\|\s*`([^`]+)`\s*\|", line)
        if symbols_row is not None:
            current_symbol_path = _normalise_path_key(symbols_row.group(1))
            continue
        symbol_sha_row = re.match(
            r"^\|\s*Symbol\s+SHA-?1\s*\|\s*`([0-9A-Fa-f]{40})`\s*\|",
            line,
        )
        if symbol_sha_row is not None and current_symbol_path is not None:
            symbol_pins.append((current_symbol_path, symbol_sha_row.group(1).lower()))
            current_symbol_path = None

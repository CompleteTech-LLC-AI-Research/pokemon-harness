...
class TcpSerialLink:
    ...
    def __init__(self, sock: socket.socket, local_rom_version: str) -> None:
        ...
        self._inbound: dict[str, queue.Queue[bytes | _ClosedSignal]] = defaultdict(
            queue.Queue
        )
    ...
    def exchange(
        ...
        if payload is _CLOSED_SIGNAL:
            self._raise_if_reader_failed()
            raise SerialLinkClosed(
                f"link closed while waiting for kind={kind!r}"
            )
        return payload

    def close(self) -> None:
        # Serialize the final BYE with ordinary frames and publish the
        # closed state only after the send attempt. The old ordering set
        # ``_closed`` first, making ``_send_frame`` reject BYE every time.

...
def _dispatch_link_tool(
    ...
    if name == "link_connect":
        ...
        try:
            ...
            # link_disconnect wins a concurrent connect attempt.  Treat it
            # as a normal, structured cancellation and release every
            # resource created before the handshake completed.  Without
            # this branch the sentinel escaped as an unhandled worker error,
            # leaking the socket and leaving the PyBoy backend attached.
            if network_session is not None:
                try:

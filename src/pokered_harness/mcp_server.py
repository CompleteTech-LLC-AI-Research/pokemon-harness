...
def _deactivate_link_hooks(session: Session, peer: Session | None = None) -> None:
    ...
    for target in sessions:
        ...
        for symbol_name in _DIRECT_LINK_HOOKS:
            ...
            except (KeyError, SessionClosedError, ValueError):
                pass
...
def main() -> None:
    ...
    * ``POKERED_PEER_ROM_PATH`` / ``POKERED_PEER_SYM_PATH`` /
    ...
    versions = None
    needs_versions = (
        primary_env.rom_sha1 is None
        or (peer_env.rom_path is not None and peer_env.rom_sha1 is None)
    )
    ...
    expected_symbol_sha: str | None = None
    if versions is not None:
        expected_symbol_sha = versions.sha1_for_symbol_path(primary_sym)
        if expected_symbol_sha is None:
            raise SystemExit(
                "no symbol SHA-1 pin matches POKERED_SYM_PATH; set "
                "POKERED_VERSIONS_PATH to a matching manifest or explicitly "
                "set POKERED_SKIP_SHA1=1 for diagnostics"
            )
    ...
    peer_expected_symbol_sha: str | None = None
    if peer_rom is not None and peer_sym is not None and versions is not None:
        peer_expected_symbol_sha = versions.sha1_for_symbol_path(peer_sym)
        if peer_expected_symbol_sha is None:
            raise SystemExit(
                "no symbol SHA-1 pin matches POKERED_PEER_SYM_PATH; set "
                "POKERED_VERSIONS_PATH to a matching manifest or explicitly "
                "set POKERED_SKIP_SHA1=1 for diagnostics"
            )
    expected_pyboy = versions.pyboy_version if versions is not None else "2.7.0"
    ...
    try:
        with contextlib.redirect_stdout(sys.stderr):
            session = Session.from_files(
                ...
                expected_symbol_sha1=expected_symbol_sha,
            ...
            if peer_rom is not None and peer_sym is not None:
                peer_session = Session.from_files(
                    ...
                    expected_symbol_sha1=peer_expected_symbol_sha,
                    expected_pyboy_version=expected_pyboy,
                )

...
def _worker(call, name):
    ...
    def run():
        try:
            call()
        except BaseException as error:  # noqa: BLE001 - thread failure collection
            errors.append(error)
...
def test_reversed_group_order_is_canonical_and_reverse_manual_order_fails_closed():
    ...
    with higher.locked():
        with pytest.raises(EmulatorOwnershipError, match="canonical order"), lower.locked():
...
class _BoundedNetworkBackendFake:
    """Minimal network adapter with the production bounded cleanup contract."""
    ...
    def start_receiver(self, **kwargs):
        self._local_core = kwargs.get("local_core")
        self._irq_callback = kwargs.get("irq_callback")
    ...
    def detach_local_core(self, *, timeout_s):
        self.detach_calls.append(timeout_s)
        self._local_core = None
        self._irq_callback = None
        self._serial_transcript_context_provider = None
        return True
...
def test_network_detach_stops_transport_without_waiting_for_emulator_owner():
    ...
    backend = _BoundedNetworkBackendFake(stopped)
...
def test_raw_network_step_owns_emulator_but_allows_cancellation(operation):
    ...
    entered, release, attempted = (threading.Event() for _ in range(3))
    backend = _BoundedNetworkBackendFake(release)
    provider = PyBoyLinkSession(network_backend=backend)
    emulator = _endpoint("network")
    session = Session(pyboy=emulator, symbols=_session()[0].symbols)

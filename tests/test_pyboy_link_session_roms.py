...
def test_yellow_pair_exchanges_bytes_after_receptionist_A_press():
    ...
    try:
        ...
        for _ in range(total_frames // step_chunk):
            # Drive the paired scheduler rather than ticking one emulator
            # through a whole chunk while its peer is frozen.  The latter
            # can leave a master in a serial wait loop and makes this smoke
            # test needlessly dependent on CPU scheduling.
            link.step(step_chunk)
...
def _party_raw_summary(session) -> dict[str, object]:
    ...
    # Keep the complete game-owned party-mon payload, not only its species
    # byte.  A species-only assertion can report a false-positive when the
    # transport drops or corrupts OT/level/HP/move data while preserving the
    # first byte.  Hex strings keep diagnostics stable and JSON-friendly.
    mon_records = [
        bytes(

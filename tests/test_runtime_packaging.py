...
def test_dev_extra_contains_matrix_screenshot_dependency() -> None:
    ...
    assert any(requirement.lower().startswith("pillow") for requirement in dev_requirements)
...
def test_pyboy_runtime_exposes_the_harness_serial_contract() -> None:
    import pyboy
    import pyboy.link as pyboy_link

    assert pyboy.__version__ == "2.7.0"
    assert pyboy.__pokered_harness_revision__ == EXPECTED_PYBOY_REVISION
    assert pyboy_link.SerialCore is Serial
    assert pyboy_link.LinkSession is not None
...
def test_vendored_runtime_contains_no_game_rom_artifacts() -> None:
    ...
    # A developer may have a full PyBoy checkout in this directory; its
    # ignored default_rom*.gb fixtures are not part of the harness package.
    # Reject every other cartridge artifact from the vendored source tree.
    allowed_pyboy_fixtures = {"default_rom.gb", "default_rom_cgb.gb"}
    forbidden = tuple(
        path
        for path in (*source.rglob("*.gb"), *source.rglob("*.gbc"))
        if path.name not in allowed_pyboy_fixtures
    )

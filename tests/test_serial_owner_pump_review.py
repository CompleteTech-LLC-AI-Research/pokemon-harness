...
def test_owner_change_revalidates_pending_edge(action):
    ...
    if action == "rearm":
        # Re-arming is a real owner-side state change.  The current serial
        # contract therefore permits the pending physical edge to proceed;
        # this deliberately failing backend proves that the resulting fault
        # is latched without mutating the shift register.

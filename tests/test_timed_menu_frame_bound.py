...
def test_a_slow_clock_truncates_the_run_and_that_is_the_measured_defect(monkeypatch, tmp_path):
    """Pin the #252 failure mode: a slow clock truncates the retained calls.

    Each owner iteration reads the clock about three times, so a 0.05 s step
    exhausts the 20-second deadline after ~400 reads. Driven by that clock the
    owner stops at ``cancelled_or_deadline`` having recorded far fewer calls than
    the frame bound authorizes. This is the #252 defect reproduced on demand
    rather than waited for: on a real contended host the same code path yields
    258 or 262 of 300.

    Asserting it here is deliberate. It documents exactly why the retention
    tests must run on a clock they control, and it fails loudly if someone later
    makes the deadline authoritative for a retention contract.
    """
    elapsed = [0.0]
    ...
    record, _, _, _ = run_owner(
        monkeypatch, tmp_path, stream=False, milestones=False, clock=slow_clock
    )
    assert record["errors"] == []
    assert record["termination"] == "cancelled_or_deadline"
    assert 0 < len(record["calls"]) < FRAME_BOUND
    assert elapsed[0] >= 20.0, "the deadline, not the frame bound, must have ended the run"


def test_a_frozen_clock_retires_the_whole_frame_bound(monkeypatch, tmp_path):
    """The repair: a clock that never advances cannot shorten a retention run.

    Same owner, same 20-second deadline, same frame bound -- but the clock does
    not move, so the deadline is never reached and the frame bound decides. This
    is what the retention tests in ``test_timed_menu_milestones`` now rely on,
    and it holds on any host regardless of speed.
    """
    record, _, _, _ = run_owner(

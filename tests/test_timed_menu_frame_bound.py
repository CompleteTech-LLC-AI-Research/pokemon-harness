...
def assert_fast_clock_hits_frame_bound(record):
    """Fail loudly when a fast-clock run stopped on the wall clock.

    A deadline-truncated run reads as a retention bug if the caller only looks at
    the call count: the owner loop leaves ``termination`` at its default when it
    exits on the wall clock, and a zero-call run then reports ``assert 0 == 300``
    (#252). A fast fake clock can never legitimately reach the deadline, so this
    guard is unconditional for it. The deliberately-slow and real-clock callers
    assert the terminal state themselves and do not use this helper.
    """
